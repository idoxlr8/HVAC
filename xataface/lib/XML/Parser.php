...
// +----------------------------------------------------------------------+
//
// $Id: Parser.php,v 1.1.1.1 2005/11/29 19:21:56 sjhannah Exp $
   ...
   /**
    ...
    function setMode($mode)
    {
        if ($mode != 'func' && $mode != 'event') {
            self::raiseError('Unsupported mode given', XML_PARSER_ERROR_UNSUPPORTED_MODE);
        }
    ...
    function _initHandlers()
    {
        ...
        switch ($this->mode) {
            ...
            case 'event':
                ...
                break;
            default:
                return self::raiseError('Unsupported mode given', XML_PARSER_ERROR_UNSUPPORTED_MODE);
                break;
        }
    ...
    function _create()
    {
        ...
        if (is_resource($xp)) {
            if ($this->tgtenc !== null) {
                if (!@xml_parser_set_option($xp, XML_OPTION_TARGET_ENCODING,
                                            $this->tgtenc)) {
                    return self::raiseError('invalid target encoding', XML_PARSER_ERROR_INVALID_ENCODING);
                }
            }
            $this->parser = $xp;
        ...
        return self::raiseError('Unable to create XML parser resource.', XML_PARSER_ERROR_NO_RESOURCE);
    }
    ...
    function setInputFile($file)
    {
        ...
        return self::raiseError('File could not be opened.', XML_PARSER_ERROR_FILE_NOT_READABLE);
    }
    ...
    function setInput($fp)
    {
        ...
        return self::raiseError('Illegal input format', XML_PARSER_ERROR_INVALID_INPUT);
    }
