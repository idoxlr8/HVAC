...
class Dataface_Relationship {
	...
	function _init(&$rel_values){
		...
		if ( array_key_exists( '__sql__', $rel_values ) ){
			...
			foreach ( $rel_values as $c1 => $c2 ){
				...
				if ( in_array( $c1, array('__sql__', '__select__', '__sort__','__domain__') ) ) continue;
	...
	function _normalizeColumns(){
		...
		for ($i=0; $i<sizeof($rel['columns']); $i++){
			...
			if ( preg_match('/^(\w+\.){0,1}\*$/', $rel['columns'][$i], $matches) ){
				...
				foreach ($temp_tables as $table){
					...
					if ( PEAR::isError($table_table) ){
						$table_table->addUserInfo("Failed to load table for table '$table' at line ".__LINE__." of file ".__FILE__.".");
						return $table_shema;
					}
			...
			// Case 2: This is a fully qualified column address.	
				...
				if ( PEAR::isError($table_table) ){
					$table_table->addUserInfo("Failed to load table for table '$table' at line ".__LINE__." of file ".__FILE__.".");
					trigger_error($table_table->toString()."\n<br>".Dataface_Error::printStackTrace(), E_USER_ERROR);
					//return $table_shema;
				}
			...
			// Case 3: This column is specified by only a column name - needs to be made absolute.
				...
				if ( PEAR::isError($name) ){
					$name->addUserInfo("Failed get absolute field name for '".$rel['columns'][$i]."' at line ".__LINE__." of file ".__FILE__.".");
				...
				if ( preg_match('/(\w+)\.(\w+)/', $name, $matches)){
					...
					PEAR::raiseError(Dataface_SCHEMA_PARSE_ERROR,null,null,null,"Error parsing table name from '$name' at ".__LINE__." of file ".__FILE__.".");
				}
			}
		}
	...
	function getSQL($getBlobs=false, $where=0, $sort=0){
		...
		if ( isset($this->_sql_generated[$where][$sort]) and $this->_sql_generated[$where][$sort] ){
			...
			if ( !isset($this->_schema['sql_without_blobs'][$where][$sort]) or !isset($this->_schema['sql_with_blobs'][$where][$sort])){
				...
				foreach ( $this->fields(true)  as $colname){
					...
					if ( in_array(strtolower($table->getType($col)), array('timestamp','datetime')) ){
						...
						$parsed['columns'][] = array('type'=>'compiled_func', 'table'=>null, 'value'=>"convert_tz(".$colFull.",'SYSTEM','".addslashes(df_utc_offset())."')", 'alias'=>$col);
					} else {
				...
				$compiler->version = 2;
				$this->_schema['sql_with_blobs'][$where][$sort] = $compiler->compile($parsed);
				
				
				foreach ($blobCols as $blobCol){
					$wrapper->removeColumn($blobCol);
				}
				$this->_schema['sql_without_blobs'][$where][$sort] = $compiler->compile($parsed);
	...
	function supportsAddExisting(){
		if ( !isset($this->addExisting) ){
			...
			if ( isset( $this->_schema['actions']['addexisting'] ) and !$this->_schema['actions']['addexisting']  ) $this->addExisting = false;
		}
	...
	function getDomainSQL(){
		...
		if ( !isset( $relationship['domain_sql'] ) ){
			...
			if ( count($domain_tables) !== 1 ){
				return PEAR::raiseError("Error calculating domain tables for relationship '".$this->_name."'.  Selected tables are {".implode(',',$relationship['selected_tables'])."} and Useless tables are {".implode(',', $uselessTables)."}.\n<br>".Dataface_Error::printStackTrace(),null,null,null,1);
			}
	...
	function getAddableValues(&$record, $filter=array()){
		...
		if ( !is_a($record, 'Dataface_Record') ){
			trigger_error("Attempt to call getAddableValues() without providing a Dataface_Record as context: ". Dataface_Error::printStackTrace(), E_USER_ERROR);
