...
class Dataface_Table {
	...
	/**
	 * Returns an associative array mapping table names to
		 ...
		 if ( isset( $this->_cache['getRelationshipsAsActions']) ){
			 ...
			 foreach ( $rkeys as $key){
				...
				$actions[$key] = array(
					...
					'condition' => '$record and $record->checkPermission("view related records", array("relationship"=>$key))',
					'order' => 1,
	...
	public static function absoluteFieldName($field, $tablenames, $db='', $columnList=null){
		...
		if ( $found == 0 ){
			...
			//echo Dataface_Error::printStackTrace();			//print_r($field); exit;
			echo Dataface_Error::printStackTrace();
