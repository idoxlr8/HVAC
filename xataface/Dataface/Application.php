...
class Dataface_Application {
	...
	/**
	 * Returns the Xataface language code for a particular locale.
	...
	/**
	 * Returns an array of all available languages in the application.
	...
	/**
	 * Returns the nav item info for a key.  This is a wrapper around the nav items defined
	...
	/**
	 * Checks whether the specified nav item is currently selected.  This is used
	...
	function getSiteTitle(){
		...
		} else return $this->parseString('{$tableLabel} - Dataface Application');
		...
		if ( !@$this->_conf['disable_session_ip_check'] ){
			...
			if ( $_SESSION['XATAFACE_REMOTE_ADDR'] != $_SERVER['REMOTE_ADDR'] ){
				$msg = sprintf(
					...
					htmlspecialchars($_SESSION['XATAFACE_REMOTE_ADDR']),
