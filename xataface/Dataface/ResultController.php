...
class Dataface_ResultController {
   ...
   function &getRecords(){
	   if ( !isset( $this->_records) ){
		   ...
		   if ( !$res and !is_array($res) ){
			   echo "An error occurred attempting to retrieve records from the database.: ".mysql_error($this->db);
			   exit;
		   }
   ...
   function getDisplayedRecords(){
   	...
   	function toHtml($prefix=''){
   		$app =& Dataface_Application::getInstance();
   		$query =& $app->getQuery();
   		if ( !isset($this->type) ) $this->type = $query['-mode'];
   	...
   	function listHtml($prefix=''){
   		...
   		for ( $i = $curr_page; $i>max(0,$curr_page-5); $i-- ){
   			...
   			if ( $this->_baseUrl ) $pages[$i] = $this->_baseUrl.'?'.substr($pages[$i], strpos($pages[$i], '?')+1);
   			$i_start -= $limit;
   		}
   		//$i = $curr_page+1;
   		$i_start = $start+$limit;
   		...
   		foreach ($pages2 as $pageno=>$link){
   			if ( $pageno == $curr_page ) $selected = ' selected';
   			else $selected = '';
   	...
   	function getLinkHtml($pos, $linkId=null,$imgURL = null, $imgAlt="Go"){
   		...
   		if ( $this->_query['-mode'] == 'browse' ){
   			//$this->_resultSet->loadSet(array( $this->_titleColumn) );
   			$titles = $this->_resultSet->getTitles(false, true, false);
   			//$data =& $this->_resultSet->indexedData();
   			$index = $pos;// - $this->_resultSet->start();
   			if ( $index >= count($titles) ) return '';
   			
   			//$title = $data[$index][$this->_titleColumn];
   			$title = $titles[$index];
   			
   		} else if ( $this->_query['-mode'] == 'list' ){
   			$from = $pos;
   	...
   	function getPrevLinkHtml($img=null){
   		if ( !isset($img) ){
   			$img = DATAFACE_URL.'/images/go-previous.png';
   		}
   		if ( $this->_query['-mode']=='browse'){
   			if ( $this->_resultSet->cursor()<=0 ) return '';
   			return /*"<b>&lt;&lt; Previous: </b>".*/$this->getLinkHtml($this->_resultSet->cursor()-1, 'prevLink', $img, '&lt;&lt;Back');
   		} else if ( $this->_query['-mode']=='list') {
   			if ( $this->_resultSet->start()<=0 ) return '';
   			return /*"<b>&lt;&lt; Previous: </b>".*/$this->getLinkHtml(
   				max( 
   					$this->_resultSet->start()-$this->_resultSet->limit(), 0), 'prevLink', $img, '&lt;&lt;Back' );
   		} else {
   			return '';
   		}
   	}
   	
   	function getNextLinkHtml($img=null){
   		if ( !isset($img) ) $img = DATAFACE_URL.'/images/go-next.png';
   		if ( $this->_query['-mode']=='browse' && $this->_resultSet->cursor()+1 < $this->_resultSet->found()){
   			return /*"<b>Next: </b>".*/$this->getLinkHtml($this->_resultSet->cursor()+1, 'nextLink', $img, 'Next&gt;&gt;')/*."<b> &gt;&gt;</b>"*/;
   		} else if ( $this->_query['-mode']=='list' and $this->_resultSet->end()+1 < $this->_resultSet->found() ) {
   			return /*"<b>Next: </b>".*/$this->getLinkHtml(
   				$this->_resultSet->end()+1, 'nextLink', $img, 'Next&gt;&gt;'
   			)/*."<b> &gt;&gt;</b>"*/;
   		} else {
   			return '';
   		}
