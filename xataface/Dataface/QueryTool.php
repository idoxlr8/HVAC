...
class Dataface_QueryTool {
	...
	function Dataface_QueryTool($tablename, $db=null, $query=null){
		...
		if ( isset($cache[$sql]) ) $this->_data['cardinality'] = $cache[$sql];
			...
			if ( !$res and !is_array($res) ) trigger_error("We had a problem with the query $sql.", E_USER_ERROR);
		...
		if ( isset($cache[$sql]) ){
			...
			if ( !$res and !is_array($res) ){
				echo $sql;
				trigger_error(mysql_error($this->_db).$sql.Dataface_Error::printStackTrace(), E_USER_ERROR);
			}
			...
			if ( !is_array($keys) || count($keys) == 0 ){
				trigger_error(
					df_translate(
						...
						'There is no primary key defined on table "'.$this->_table->tablename.'". Please define a primary key.'.Dataface_Error::printStackTrace(),
						array('table'=>$this->_table->tablename, 'stack_trace'=>Dataface_Error::printStackTrace())
						),
					E_USER_ERROR
					);
			}
			...
			if ( !$res and !is_array($res) ){
				...
				if ( !$res and !is_array($res) )
					...
					trigger_error(
						...
						.mysql_error($this->_db)."\n<br>".$sql.Dataface_Error::printStackTrace(), E_USER_ERROR);
			}
	...
	function &loadCurrent($columns=null, $loadText=true, $loadBlobs=false, $loadPasswords=false){
		...
		if ( sizeof( $unloaded ) > 0 ){
			...
			//echo $sql;
			if ( PEAR::isError($sql) ){
				trigger_error($sql->toString(). Dataface_Error::printStackTrace(), E_USER_ERROR);
			}
			...
			if ( !$res and !is_array($res) ){
				...
				$res = $this->dbObj->query($sql, $this->_db, null,true /* as array */);
				if ( !$res and !is_array($res) ){
					echo df_translate('scripts.Dataface.QueryTool.loadCurrent.ERROR_COULD_NOT_LOAD_CURRENT_RECORD',"Error: Could not load current record: ").mysql_error( $this->_db)."\n$sql";
					exit;
				}
			}
			if (count($res) <= 0 ){
