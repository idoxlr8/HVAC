...
class Dataface_DeleteForm extends HTML_QuickForm {
	...
	function _build(){
		...
		if ( !$this->_isBuilt ){
			...
			if ( isset( $this->_query['-delete-one'] ) ){
				...
				if ( !$res ){
					echo df_translate('scripts.Dataface.DeleteForm._build.ERROR_TRYING_TO_FETCH',"Error trying to fetch element to be deleted.: ").mysql_error($this->_db);
					exit;
				}
				if ( mysql_num_rows($res)==0 ) {
					echo df_translate('scripts.Dataface.DeleteForm._build.ERROR_NO_RECORD_SELECTED',"No record is currently selected so no record can be deleted.");
					exit;
				} else {
	...
	function display(){
		...
		if ( isset( $this->_query['-delete-one'] ) ){
			...
			if ( !$res ){
				echo df_translate('scripts.Dataface.DeleteForm.display.ERROR_ESTIMATING',"Error estimating number of rows that will be deleted: "). mysql_error($this->_db);
				exit;
			}
