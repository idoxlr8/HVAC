...
class Dataface_IO {
		...
		if ( PEAR::isError($res) ){
			...
			$res->addUserInfo(
				df_translate(
					...
					"Error while saving record of table '".$this->_table->tablename."' in Dataface_IO::write() on line ".__LINE__." of file ".__FILE__,
					array('tablename'=>$this->_table->tablename,'line'=>__LINE__,'file'=>__FILE__)
					)
				);
			//$record->vetoSecurity = $oldVeto;
			return $res;
		...
		if ( $rows > 1 ){
			...
			$err = PEAR::raiseError(
				Dataface_LanguageTool::translate(
					...
					The query used was '$sql'.  On line ".__LINE__." of file ".__FILE__,
					/* i18n parameters */
					array('table'=>$this->_table->tablename, 'line'=>__LINE__, 'file'=>__FILE__,'sql'=>$sql)
				),
				DATAFACE_E_IO_ERROR
			);
		...
		if ( PEAR::isError($exists) ){
			$exists->addUserInfo(
				df_translate(
					...
					"Attempt to update record with incomplete information. On line ".__LINE__." of file ".__FILE__,
		...
		if ( !$exists ){
			return PEAR::raiseError(
				df_translate(
					...
					"Attempt to update record that doesn't exist in _update() on line ".__LINE__." of file ".__FILE__,
					array('line'=>__LINE__,'file'=>__FILE__)
		...
		if ( PEAR::isError($sql) ){
			$sql->addUserInfo(
				df_translate(
					...
					"Error generating sql for update in IO::_update() on line ".__LINE__." of file ".__FILE__,
					array('line'=>__LINE__,'file'=>__FILE__)
					)
				);
			return $sql;
		...
		if ( strlen($sql) > 0 ){
			...
			if ( !$res || PEAR::isError($res) ){
				...
				trigger_error(
					...
					.mysql_error($s->db).Dataface_Error::printStackTrace(), E_USER_ERROR);
			}
	...
	function _insert(&$record, $tablename=null, $secure=false){
		...
		if ( !$res || PEAR::isError($res)){
			...
			trigger_error(
				...
				.(PEAR::isError($res)?$res->getMessage():mysql_error(df_db())).": SQL: $sql".Dataface_Error::printStackTrace(), E_USER_ERROR);
		}
		...
		if ( PEAR::isError($rel) ){
			$rel->addUserInfo(
				df_translate(
					...
					"Error obtaining relationship $relname in IO::_writeRelationship() on line ".__LINE__." of file ".__FILE__,
					array('relname'=>$relname,'line'=>__LINE__,'file'=>__FILE__)
					)
				);
			return $rel;
		}
		...
		if ( count($tables) == 0 ){
			return PEAR::raiseError(
				Dataface_LanguageTool::translate(
					...
					/* default error message */
					"Error writing relationship '$relname'.  No tables were selected on line ".__LINE__." of file ".__FILE__,
					/* i18n parameters */
		...
		if ( PEAR::isError( $records) ){
			$records->addUserInfo(
				df_translate(
					...
					"Error getting related records in IO::_writeRelationship() on line ".__LINE__." of file ".__FILE__,
					array('line'=>__LINE__,'file'=>__FILE__)
					)
				);
			return $records;
				...
				if ( in_array(mysql_errno($this->_table->db), array(MYSQL_ER_DUP_KEY,MYSQL_ER_DUP_ENTRY)) ){
					...
					$err = PEAR::raiseError(DATAFACE_TABLE_SQL_ERROR, null,null,null, 
						df_translate(
							...
							"Error performing query '$current_query' on line ".__LINE__." of file ".__FILE__.": <br/>",
							array('line'=>__LINE__,'file'=>__FILE__,'current_query'=>$current_query)
							)
		...
		if ( PEAR::isError($sql) ){
			$sql->addUserInfo(
				df_translate(
					...
					"Error generating sql in ShortRelatedRecordForm::save() on line ".__LINE__." of file ".__FILE__,
					array('line'=>__LINE__,'file'=>__FILE__)
					)
				);
			return $sql;
	...
	function copy(&$sourceRecord, &$destParent, $destRelationship=null, $deepCopy=false){
		trigger_error("The method ".__METHOD__." is not implemented yet.".Dataface_Error::printStackTrace(), E_USER_ERROR);
	}
			...
			if ( PEAR::isError( $res ) ){
				$res->addUserInfo(
					df_translate(
						...
						"Error while firing event '$name' on table '".$this->_table->tablename."' in Dataface_IO::write() on line ".__LINE__." of file ".__FILE__,
						array('name'=>$name,'tablename'=>$this->_table->tablename, 'line'=>__LINE__,'file'=>__FILE__)
						)
					);
				$record->vetoSecurity = $oldVeto;
				return $res;
